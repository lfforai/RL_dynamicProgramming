...
class critic_train_Net(tf.keras.Model):#critic网络,确定动作值函数
    def __init__(self,**kwargs):
    ...
    def train(self,x_train,action_train,y_ture,size):
        optimizer = tf.keras.optimizers.Adam(learning_rate=2*1e-3)
        self.compile(optimizer, loss=tf.keras.losses.MeanSquaredError())
        self.fit(x=[x_train,action_train], y=y_ture, epochs=1,batch_size=size)
...
class actor_train_Net(tf.keras.Model):#critic网络,确定动作值函数
    def __init__(self,**kwargs):
    ...
    def train(self,x_train,y_ture,size):
        ...
        self.fit(x_train, y_ture, epochs=1,batch_size=size)
...
class DDPG:
    def __init__(self,filepath="/root/model/",**kwargs):
    ...
    def action(self,state):
        # print(self.Qvalue_eval(np.array([state]))[0])
        result= np.clip(np.random.normal(self.Qvalue_eval(np.array([state]))[0].numpy(),self.var), -2.0, 2.0)#返回当前状态所确定的动作，使用可训练参数
        # print(result)
        # print("--------------")
        return result
    ...
    def DDPG_train(self):
            ...
            for e  in range(self.N):#进行迭代的次数，每次迭代进行
                ...
                self.memory.clear()
                state_now = self.env.reset()
                for T in range(self.mini_memory_len+int(self.mini_memory_len/3)):
                    if self.memory.__len__()<self.mini_memory_len:#如果样本量不足，循环采集
                        self.env.render()
                        if e==0:
                        ...
                        if done:
                            ...
                            self.memory.append([state_now,state_next,reward,action_now,done])
                            state_now=state_next
                    elif self.memory.__len__()==self.mini_memory_len:#当新样本满的时候替换掉队列中的老样本
                        ...
                        # position,_=state_next
                        # reward=reward+(abs(position+0.5)/0.95-0.5)*0.01
                        # print(state_next,reward,(abs(position+0.5)/0.95-0.5)*0.01)
                        if done:
                            self.memory.append([state_now,state_next,reward,action_now,done])
                        ...
                        temp_memory=temp_memory[:int(self.mini_memory_len*1.0/3.0)]
                        x_train=[]
                        y_critic_train=[]
                        action_vector=[]
                        ...
                        self.Qvalue_train.train(x_train,action_vector,y_critic_train,32)
                        ...
                        self.actor_train.train(x_train,y_critic_train,32)
